function Class_ST_WB_frec_suavizado_gauss_DTW_Sin_restriccion_2clases(total_bd_ts,porcentaje_train,k,sigma)
%función que obtiene la clasificación de series de tiempo con suavizado gaussiano en el dominio de frecuencias utilizando
...
%Cross fold validation con porcentaje en TRAIN y TEST
   ...
   for i=1:10
     ...
     [TRAIN,TEST]=Get_Train_Test_Data_WB_2classes_general(new_filtered_db,porcentaje_train);
     [classified_rate,matriz_confusion]=DTW_KNN_classification_frec_gauss_2clases_Sin_restriccion(TRAIN,TEST,1,3,k);
